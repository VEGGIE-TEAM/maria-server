...
def handle_data():
    if request.method == 'GET':
        try:
            nama_sayur = request.args.get('nama_sayur')
            nama_pasar = request.args.get('nama_pasar')
            ...
            # Retrieve data from the 'deteksi' table based on the provided 'id_sayur' and 'id_pasar'
            cursor.execute("SELECT * FROM deteksi WHERE id_sayur = %s AND id_pasar = %s",
                           (id_sayur, id_pasar))
            ...
            for row in results:
                data.append({
                    ...
                    'id_pasar': row[2],
                    'gambar_sayur': row[3],
                    'tanggal_input': row[4].strftime('%Y-%m-%d %H:%M:%S'),
                    'hasil_deteksi': row[5]
                })
